    ...
    try {
        ...
        if (useTestnetToggle) {
            useTestnetToggle.addEventListener('change', function() {
                state.spotTrading.useTestnet = this.checked;
                addLogMessage(`Switched to ${state.spotTrading.useTestnet ? 'Testnet' : 'Mainnet'} API`);
            });
            
            // Set initial state
            useTestnetToggle.checked = state.spotTrading.useTestnet;
        }
        ...
        if (livePositionSizeSlider && livePositionSizeValue) {
            livePositionSizeSlider.addEventListener('input', function() {
                ...
                state.spotTrading.positionSize = parseInt(value);
            });
        }
...
function loadSettingsFromLocalStorage() {
    try {
        ...
        if (apiConfig) {
            ...
            state.liveTrading.apiKey = parsedConfig.apiKey || '';
            state.liveTrading.apiSecret = parsedConfig.apiSecret || '';
            state.useTestnet = parsedConfig.useTestnet !== undefined ? parsedConfig.useTestnet : true;
        }
    } catch (error) {
...
function disableFormInputs(disabled) {
    try {
        const formInputs = [
            ...
            'use-testnet-toggle', 'initial-capital', 'position-size'
        ];
...
// Last modification: 2025-06-09 16:44:56 UTC by gelimorto2
...
async function fetchHistoricalData(symbol, interval, limit = 500) {
    try {
        ...
        const baseUrl = state.useTestnet ? BINANCE_TESTNET_API_URL : BINANCE_API_URL;
    ...
    try {
        ...
        if (chartUpdateFrequency) {
            ...
            if (state.isTrading && state.interval) {
                clearInterval(state.interval);
                if (state.liveTrading.enabled) {
                    state.interval = setInterval(pollForLiveData, frequencyValue);
                } else {
...
function showAPIConfigModal() {
    ...
    if (modal) {
        ...
        if (apiKeyInput) apiKeyInput.value = state.liveTrading.apiKey || '';
        if (apiSecretInput) apiSecretInput.value = state.liveTrading.apiSecret ? '********' : '';
        if (testnetToggle) testnetToggle.checked = state.useTestnet;
...
// Helper function to get the latest signal from the strategy
function getLatestSignal(priceData, indicators) {
    if (!priceData.length || !indicators) return null;
    ...
    try {
        ...
        if (livePositionSizeSlider && livePositionSizeValue) {
            livePositionSizeSlider.addEventListener('input', function() {
                const value = this.value;
                livePositionSizeValue.textContent = value + '%';
                state.liveTrading.positionSize = parseInt(value);
            });
        }
...
// Current Date and Time (UTC): 2025-06-09 16:57:07
// Current User's Login: gelimorto2
...
function startActualLiveTrading() {
    ...
    addLogMessage(`Started LIVE trading for ${state.symbol} on ${state.useTestnet ? 'TESTNET' : 'MAINNET'}`);
    sendAlert('Live Trading Started', 'Bot has started live trading in ' + (state.useTestnet ? 'testnet' : 'mainnet') + ' mode. Please monitor carefully.', 'warning');
}
...
function emergencyStop() {
    ...
    try {
        ...
        if (state.liveTrading.enabled && state.liveTrading.apiKey) {
            // If in live trading mode with API, we need to close the position via API
            // In a real implementation, you would call the API to close positions
            // For this demo, we'll just close the current position
            if (state.currentPosition) {
...
// Current Date and Time (UTC): 2025-06-09 16:53:29
// Current User's Login: gelimorto2
...
document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        const useTestnetToggle = safeGetElement('use-testnet-toggle');
        if (useTestnetToggle) {
            useTestnetToggle.checked = state.useTestnet;
            useTestnetToggle.addEventListener('change', function() {
                state.useTestnet = this.checked;
                addLogMessage(`Switched to ${state.useTestnet ? 'Testnet' : 'Mainnet'} API`);
        ...
        if (!scriptExists) {
            console.warn('TradingView script not found in the document');
            addLogMessage('TradingView not available, using Plotly charts only', true);
            
            // Make sure toggle button reflects this
            const toggleChartBtn = safeGetElement('toggle-chart-btn');
            if (toggleChartBtn) {
                toggleChartBtn.textContent = 'TradingView unavailable';
            ...
            // Try to load TradingView with a timeout to prevent blocking
            let tvCheckAttempts = 0;
            const checkTradingView = setInterval(function() {
                tvCheckAttempts++;
                if (typeof TradingView !== 'undefined') {
                    clearInterval(checkTradingView);
                    addLogMessage('TradingView library loaded');
                    
                    // Only initialize if we're in TradingView mode
                    if (state.usingTradingView) {
                        initTradingViewWidget();
                    }
                } else if (tvCheckAttempts >= 5) {
                    // Give up after 5 attempts
                    clearInterval(checkTradingView);
                    addLogMessage('TradingView library failed to load, using Plotly charts only', true);
        ...
        addLogMessage(`System initialized by user: gelimorto2 at 2025-06-09 16:41:15 UTC`);
    } catch (error) {
...
// Function to reload data with current settings
function reloadDataWithSettings() {
    showLoading();
    showStatusIndicator('Loading data...', 'info');
    
    // Fetch initial data with new settings
    fetchHistoricalData(state.symbol, state.timeframe, 500)
        .then(data => {
            hideLoading();
            hideStatusIndicator();
            ...
            // Update bot status
            updateBotStatus('idle', 'Data loaded - Ready to start trading');
            
            addLogMessage(`Loaded data for ${state.symbol} on ${state.useTestnet ? 'Testnet' : 'Mainnet'}`);
        })
        .catch(error => {
            hideLoading();
            hideStatusIndicator();
...
// Add remaining helper functions and event handlers
// (Implementations for other functions like initTradingViewWidget, updatePriceChart, etc.)

// For a full implementation, the file would include all the necessary helper functions
// and event handlers that were in the original code, modified to work with spot trading and the testnet option.
